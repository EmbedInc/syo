{   SYO_P_TAG_END (MFLAG,TAG)
*
*   Close a tagged region started with SYO_P_TAG_START.  MFLAG is the syntax matched
...
*   match, then state will be restored to what it was when SYO_P_TAG_START was
...
module syo_p_tag_end;
define syo_p_tag_end;
...
begin
  ...
  if mflag = syo_mflag_yes_k
    then begin                         {syntax matched, finish tag}
      if tag <= 0 then begin
        writeln ('TAG value out of range in SYO_P_TAG_END.');
        sys_bomb;
        end;
