{   Subroutine SYN_GET_CHAR_LINE (CHAR_HANDLE, ICHAR, CNUM, SLINE, LNUM, TNAM)
*
...
*   positive, and special flag values are negative.  Use constants SYN_ICHAR_xxx_K
...
module syn_get_char_line;
define syn_get_char_line;
...
var
  ch: syn_char_t;                      {local handle to character actually used}
...
begin
  ...
  while                                {loop until find a real character}
    ...
    if ch.crange_p^.next_p = nil then begin {hit end of input stream ?}
      ichar := syn_ichar_eod_k;        {indicate end of input stream}
      return;
      end;
