...
*   are always negative.  Use the constants SYO_ICHAR_xxx_K to identify the special
*   flag values.  Currently (1990/8/19) these are:
*
*   SYO_ICHAR_EOL_K - End of a line.
*   SYO_ICHAR_EOF_K - End of a file.
*   SYO_ICHAR_EOD_K - End of all input data.
}
module syo_p_get_ichar;
define syo_p_get_ichar;
...
var
  cr_p: syo_crange_p_t;                {pointer to new character range descriptor}
...
begin
  if                                   {exhausted current character range ?}
    ...
    if next_char.crange_p^.next_p = nil
      then begin                       {this was last char range in chain}
        syo_range_next (cr_p);         {get pointer to next input char range}
        if cr_p = nil then begin       {no more input data at all ?}
          ichar := syo_ichar_eod_k;    {pass back "end of data"}
          return;
          end;
  ...
  case charcase of
syo_charcase_down_k: begin             {make sure all letters are lower case}
      if (ichar >= 65) and (ichar <= 90) {upper case character ?}
        then ichar := ichar + 32;
      end;
syo_charcase_up_k: begin               {make sure all letters are upper case}
