{   Subroutine SYN_GET_TAG_STAT (TAG, STRING_HANDLE, STAT)
*
...
*   TAG contains an application tag (> 0) or SYN_TAG_END_k.  STAT is
*   set to an apporpriate error status on syntax error.
}
module syn_get_tag_stat;
define syn_get_tag_stat;
...
begin
  ...
  syn_get_tag (tag, string_handle);    {get the tag}
  if                                   {not an error ?}
      (tag >= 1) or                    {app tag ?}
      (tag = syn_tag_end_k)            {end of this syntax level ?}
...
*   syntax tree has been reached.
  ...
  syn_get_char_line (                  {get info about the error character}
...
syn_ichar_eol_k: begin                 {error character was end of line}
      sys_stat_set (syn_subsys_k, syn_stat_syerr_eol_k, stat);
...
syn_ichar_eof_k: begin                 {error character was end of file}
      sys_stat_set (syn_subsys_k, syn_stat_syerr_eof_k, stat);
      sys_stat_parm_vstr (tnam, stat);
      end;
syn_ichar_eod_k: begin                 {error character was end of data}
      sys_stat_set (syn_subsys_k, syn_stat_syerr_eod_k, stat);
      end;
otherwise                              {error on regular printable character}
    sys_stat_set (syn_subsys_k, syn_stat_syerr_at_k, stat);
    sys_stat_parm_int (lnum, stat);
    sys_stat_parm_vstr (tnam, stat);
    sys_stat_parm_vstr (sline, stat);
