...
*   the syntax tree looks like.
  ...
  tree_frame_tag_t = record            {declare tagged syntax region}
    ...
    fchar: syo_char_t;                 {handle to first character of string}
    lchar: syo_char_t;                 {handle to last character of string}
    end;
...
*   End of templates for syntax tree frames.
  ...
  stack_frame_cpos_t = record          {stack frame for saving input stream state}
    next_char: syo_char_t;             {handle to next input stream character}
    charcase: syo_charcase_k_t;        {current character case conversion flag}
...
{
*   Common block accessed by SYO_ routines.
}
var (syo_common)
...
*   Variables used when parsing.
  ...
  syo_preproc_p: syo_preproc_p_t;      {points to pre-processor routine}
  next_crange_seq_n: sys_int_machine_t; {sequence number for next crange descriptor}
  file_p: syo_file_p_t;                {pointer to data about current input file}
  file_name_p: syo_file_p_t;           {pnt to data about "pretend" input file}
  tframe_p: tree_frame_down_p_t;       {pointer to curr syntax level header frame}
  start_char: syo_char_t;              {char position before first parse routine}
  next_char: syo_char_t;               {handle to next input stream character}
  far_char: syo_char_t;                {handle to farthest character requested}
  charcase: syo_charcase_k_t;          {current character case conversion flag}
  made_tag: boolean;                   {TRUE if made tag at or below current level}
...
procedure syo_next_tree_pos;           {advance traversal to next syntax tree frame}
  extern;
...
procedure syo_range_next (             {create and return pointer to next char range}
  out     p: syo_crange_p_t);          {pointer to new char range, NIL = end of input}
  extern;
