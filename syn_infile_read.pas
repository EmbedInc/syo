{   Subroutine SYN_INFILE_READ (LINE_P, STAT)
*
...
module syn_INFILE_READ;
define syn_infile_read;
...
var
  ...
  f_p: syn_file_p_t;                   {pnt to real source file descriptor}
  f_name_p: syn_file_p_t;              {pnt to "pretend" source file descriptor}
  ...
  if file_p = nil
    then begin                         {no current input file exists}
      ...
      if file_p^.eof then begin        {already got to end of current file ?}
        syn_infile_pop;                {pop back to previous input file}
      ...
      if file_eof(stat)
        then begin                     {we just hit end of this file}
          ...
          n_spec := n_spec + 1;        {one more special char needed at end of line}
          syn_infile_pop;              {close this file and pop back to previous}
          end
